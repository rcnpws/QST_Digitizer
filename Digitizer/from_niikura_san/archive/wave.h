// wave.h
// $Id: wave.h 2019-06-28 18:52:59 niikura Exp $
...
void WaveForm::PSA()
{
  ...
  // -- find 40 and 50Hz timings
  t0=0;
  for(int j=0;j<300;j++){
    if(baseline[ch2det[3]]-wave[ch2det[3]][j]>1000){
      t0=j;
      break;
    }
  }
  t1=0;
  for(int j=0;j<300;j++){
    if(baseline[ch2det[4]]-wave[ch2det[4]][j]>1000){
      t1=j;
      break;
    }
  }
  ...
  // -- timestamp
  if(TSprev>TS) TS1++;
  TimeStamp = (TS1*TMath::Power(2,31)+TS)*8 + t1*2;
  TSprev=TS;
...
{
  // c1->cd(1);
  Double_t offset[5]={1000,0,100,200,300};
  Double_t gain[5]  ={1,1,10,1,1};
  for(int i=0;i<5;i++){
    for(int j=0;j<300;j++){
      gra[i]->SetPoint(j,j,-gain[i]*(wave[ch2det[i]][j]-baseline[i])+offset[i]);
    }
