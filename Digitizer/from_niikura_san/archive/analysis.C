// analysis.C for oscilloscope mode only
// $Id: analysis.C 2019-06-28 18:05:11 niikura Exp $
...
int main(int argc, char *argv[])
{
  ...
  while((option=getopt(argc,argv,"osSd"))!=-1){
    switch(option){
    case 'o':
      oscillo_flag=1;
      break;
  ...
  if(oscillo_flag==1)
    cout << "       oscilloscope mode ON (No save mode)" << endl;
  if(hist_flag==1)
  ...
  // ===== JPARC 201803 Beam tuning
  // if(RunNumber%2==0)
  //     inputfilename = Form("/data00/JPARC1803_00/run%04d.dat",RunNumber);
  // else
  //   inputfilename = Form("/data01/JPARC1803_01/run%04d.dat",RunNumber);
  // ===== for JPARC 201803
  // inputfilename = Form("/backup/JPARC1803/run%04d.dat",RunNumber);
  // ===== for RAL
  inputfilename = Form("./rawdata/run%04d.dat",RunNumber);
  ...
  while(1){
    ...
    if(fin->eof()){
      cout << Form("\r%d events analyzed (about %3.1f min data)",
      		   iWaveEvent, iWaveEvent/50./60.) << flush;
      cout << endl;
      if(hist_flag){
	wf.DrawHist();
      }
	...
	ge1.TT     = TimeTag;
	ge1.Eraw   = energy;
	Double_t ee = energy+rndm->Rndm();
	

	Double_t p2= 1.32499541892662e-08;
	Double_t p1= 0.201488448007906;
	Double_t p0= 0.148803388569986;
	ge1.Ecal   = p2*ee*ee + p1*ee + p0;
	...
	ge2.Eraw   = energy;
	Double_t ee = energy+rndm->Rndm();
	ge2.Ecal   = ( -3.0448e-7*ee*ee + 0.26834*ee - 0.4323 )*511./508.6;
