...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	while(!feof(f_ini)) {
		...
		if (strstr(str, "DECIMATION_FACTOR")!=NULL) {
			read = fscanf(f_ini, "%" SCNu16, &WDcfg->DecimationFactor);
			continue;
		}
...
//printf("dc offset is set to %d for channel %d in WDcfg\n", WDcfg->DCoffset[ch],ch);
			continue;
            }
		}
		...
		if (strstr(str, "GRP_CH_DC_OFFSET")!=NULL) {
		  float dc[8];
		  read = fscanf(f_ini, "%f,%f,%f,%f,%f,%f,%f,%f",
