...
*   \return  0=success; -1=error */
/* --------------------------------------------------------------------------------------------------------- */
int SaveWaveform(int b, int ch, int trace, int size, int16_t *WaveData, FILE *fh)
...
int OpenWaveformFile(int b, int ch, int trace, FILE *fh)
{
    //FILE *fh;
    //int i;
    char filename[20];

    sprintf(filename, "./data/Waveform_%d_%d_%d.txt", b, ch, trace);
    fh = fopen(filename, "w");
    if (fh == NULL)
        return -1;
    return 0;
...
int CloseWaveformFile(int b, int ch, int trace, FILE *fh)
{
    //FILE *fh;
    //int i;
    char filename[20];
...
/* --------------------------------------------------------------------------------------------------------- */
void PrintInterface() {
	printf("\ns ) Start acquisition (no save mode) \n");
	printf("S ) Stop acquisition\n");
	//	printf("r ) Restart acquisition\n");
	printf("q ) Quit\n");
	printf("m ) Monitor IC temperature\n");
	printf("g ) Ge paramter tuning (only Ge tuning mode)\n");
	//	printf("t ) Send a software trigger\n");
	//	printf("h ) Save Histograms to file\n");
	printf("w ) data taking (Use this during the experiment) \n\n\n");
...
{
  for(int i = 0; i < 32; i++) {
    printf("\x1b[41m Error alert!!! Error alert !!! Error alert !!!\x1b[0m\n");
  }
}
