...
// $Id: wave.C 2018-02-19 20:03:05 daq Exp $
...
int main(int argc, char *argv[])
{
  ...
  while((option=getopt(argc,argv,"hoOd"))!=-1){
    switch(option){
    case 'h':
      hist_flag=1;
      break;
  ...
  if(hist_flag)
    cout << "       histgram mode ON" << endl;
  if(oscillo_flag==1)
    cout << "       event-by-event oscilloscope mode ON" << endl;
  if(oscillo_flag==2)
    cout << "       continuous oscilloscope mode ON" << endl;
  if(debug)
