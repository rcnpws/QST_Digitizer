...
int main(int argc, char *argv[])
{
  ...
  //        input files          //
  //*****************************//
  const char* const ConfigFile1 = "./WaveDumpConfig1.txt";
  const char* const ConfigFile2 = "./WaveDumpConfig2.txt";
  ...
  if(WDcfg1.Nbit == 8) {
    ret = CAEN_DGTZ_AllocateEvent(handle[NB_WF], (void**)&Event8);
  } else if (BoardInfo1.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) {
    ret = CAEN_DGTZ_AllocateEvent(handle[NB_WF], (void**)&Event742);
  } else {
    ret = CAEN_DGTZ_AllocateEvent(handle[NB_WF], (void**)&Event16);  
  }
  ...
  //@ changed by saito
  ret  = CAEN_DGTZ_MallocReadoutBuffer(handle[NB_WF], &buffer,&AllocatedSize); 

  ret |= CAEN_DGTZ_MallocReadoutBuffer(handle[NB_PHA], &buffer3, &AllocatedSize);
  ret |= CAEN_DGTZ_MallocDPPEvents(handle[NB_PHA], (void**)PHAEvents, &AllocatedSize); 
  ret |= CAEN_DGTZ_MallocDPPWaveforms(handle[NB_PHA], (void**)&PHAWaveform, &AllocatedSize); 
  if (ret) {
  ...
  */
  ...
  while(!Quit) {
	...
	/* set output filename */
	sprintf(datafile_path,"/data0%d/JPARC1711_0%d",runnumber%2,runnumber%2); //change HDD to write
