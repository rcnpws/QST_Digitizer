...
// $Id: wave.C 2017-12-13 18:34:54 daq Exp $
...
int main(int argc, char *argv[])
{
  ...
  //char *inputfilename = Form("/data02/RAL1709_02/run%04d.dat",RunNumber); //for last calib after beamtime
  
  // if(RunNumber>187)
  // inputfilename = Form("./data%02d/run%04d.dat",RunNumber%2,RunNumber);
  // else
  //inputfilename = Form("./data00/run%04d.dat",RunNumber);
  //inputfilename = Form("/data02/RCNP1702_02/run%04d.dat",RunNumber);
  ...
  // inputfilename = Form("/backup/JPARC1711/run%04d.dat",RunNumber);
  ...
  while(1){
    ...
    if(!fin->eof() && dd==magic_number_ge){
      ...
      if(channel==0){
      	GeE = 0.14197 + 0.302122*((Double_t)rawGe+gRandom->Uniform(0,1));
      	// GeE = 1.08  + 0.40419*((Double_t)rawGe+gRandom->Uniform(0,1));
	// GeE = 0.9969*GeE - 0.7118;
	tree2->Fill();
      }else if(channel==2){
      	GeE = -0.924758 + 0.304869*((Double_t)rawGe+gRandom->Uniform(0,1));
      	// GeE=-0.283 + 0.25942*((Double_t)rawGe+gRandom->Uniform(0,1));
	tree3->Fill();
      }else if(channel==4){
      	GeE= 0.000 + 1.00000*((Double_t)rawGe+gRandom->Uniform(0,1));
	tree4->Fill();
      }else{
      	GeE=-9999;
      }
	...
	dd = common_decoder(fin,bn*16+ch);
      ...
      if(detector_type[bn*16+ch]==1 || detector_type[bn*16+ch]==2){
	// ana_seamine(bn*16+ch);
	ana_plastic(bn*16+ch);
      }
    }
  ...
  if(oscillo_flag==0){
    ...
    tree4->Write("",TObject::kOverwrite);
