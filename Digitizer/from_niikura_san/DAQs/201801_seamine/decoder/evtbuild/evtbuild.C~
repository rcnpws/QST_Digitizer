...
// 
// $Id: evtbuild.C 2017-09-21 01:44:47 niikura Exp $
...
int main(int argc, char** argv)
{
  ...
  TH1D *h1 = new TH1D("h1","h1",2000,0,20000);
  ...
  for(ULong64_t i=0;i<imax;i++){
    ...
    while(1){
      ...
      if(time1<time0){
    	start_evt1=j1;
    	continue;
      } else if(time1-time0>19500.){
    	break;
      } else {
	// cout << Form("%lld",time1-time0) << endl;
	h1->Fill(time1-time0);
	if((time1-time0)>100 && (time1-time0)<2000){
	  // cout << "**" << endl;
	  gamma1p->Fill(fData2.GetEnergy(j1-1));
	}else if((time1-time0)>2000){
