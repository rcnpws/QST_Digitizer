...
// $Id: detector.h 2018-01-26 15:43:56 niikura Exp $
...
Int_t detector_type[40] = {1, 1, 1, 1,   2, 2, 2, 2, // BN0
			   2, 2, 2, 4,   0, 0, 0, 0, // BN0
			   3, 3, 3, 3,   3, 3, 3, 3, // BN1
			   3, 3, 3, 3,   3, 3, 3, 3, // BN2
			   3, 3, 3, 3,   3, 0, 0, 0};// BN2
...
Int_t threshold[40] = {  0,   0,   0,   0,   100, 100, 100, 100,
...
Double_t tof_offset[40] = { 0.0,  0.0,  0.0,  0.0,
...
void DefineTree(Bool_t waveform_flag){
  ...
  tree0->Branch("QDCe0",&QDCe[ 0],"QDCe[16]/D");
  tree1->Branch("QDCe1",&QDCe[16],"QDCe[16]/D");
...
void ana_plastic(Int_t chid)
{
  ...
  for(int i=90;i<150;i++){
    QDCe[chid]+=(wave[chid][i]-baseline[chid])/1000.;
  }
  for(int i=150;i<225;i++){
    QDCs[chid]+=(wave[chid][i]-baseline[chid])/1000.;
  }
